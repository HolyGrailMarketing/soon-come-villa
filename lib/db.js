...
// `cache: 'no-store'` is essential under Next.js: the neon HTTP driver uses
// fetch(), and Next caches fetch responses by default — without this, reads
// (availability, bookings, units) return stale snapshots.
export const sql = neon(connectionString, { fetchOptions: { cache: 'no-store' } });
