 ...
 */
export async function mountDimePay({ order_id, data, clientId, total, currency, test, onSuccess, mountId = 'dimepay-widget' }) {
  const mount = document.getElementById(mountId);
  if (mount) { mount.style.display = 'block'; mount.replaceChildren(); } // clear any prior render
  ...
  initPayment({
    ...
    client_id: clientId,
    data,
    // Sandbox vs production routing is driven by this flag, NOT the signed JWT.
    test: !!test,
