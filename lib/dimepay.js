...
async function restCall(method, path, body) {
  ...
  const res = await fetch(`${REST_BASE}${path}`, {
    ...
    headers: {
      ...
      // DimePay REST auth header uses the PUBLIC client key (ck_), per the SDK
      // README. Request bodies are { data: <JWT signed with the secret key> }.
      client_key: clientId,
    },
  ...
  if (!res.ok) {
    console.error('DimePay REST error', method, path, res.status, text?.slice(0, 500));
...
/** PUT /payments/refund — full or partial. Body is a signed-data JWT. */
export function refundPayment({ txnId, amount, currency }) {
  const data = jwt.sign({ transaction_id: txnId, id: txnId, amount, currency }, secretKey,
    { algorithm: 'HS256', expiresIn: '1h' });
  return restCall('PUT', '/payments/refund', { data });
}
...
/**
 * Verify and decode a DimePay webhook. DimePay POSTs { payload: <JWT> } where the
 * JWT is signed with the merchant secret key — so a successful verify both decodes
 * the event AND authenticates that it came from DimePay.
 */
export function verifyWebhook(token) {
  if (!secretKey) throw new Error('DIMEPAY_SECRET_KEY is not configured');
