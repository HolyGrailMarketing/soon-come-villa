...
export async function POST(request) {
  ...
  if (!exp) return json({ error: 'unknown unit' }, 400);
  ...
  if (!unitRow) return json({ error: 'unit not bookable' }, 400);

  // Ballroom: single exclusive day -> [date, date+1).
  if (unitRow.kind === 'ballroom') {
    const date = body.date || checkIn;
    if (!YMD.test(date || '')) return json({ error: 'date is required' }, 400);
    checkIn = date;
    checkOut = nextDay(date);
  }

  if (!YMD.test(checkIn || '') || !YMD.test(checkOut || '') || checkOut <= checkIn) {
    return json({ error: 'valid checkIn/checkOut required' }, 400);
  }
  const today = new Date().toISOString().slice(0, 10);
  if (checkIn < today) return json({ error: 'check-in is in the past' }, 400);
  ...
  if (Number.isNaN(g) || g < 1 || g > unitRow.max_guests) {
    return json({ error: `guests must be 1–${unitRow.max_guests}` }, 400);
  }

  // Server-side amount — never trust any client-sent price.
  let amount;
  if (unitRow.kind === 'ballroom') {
    if (unitRow.flat_day_rate == null) return json({ error: 'ballroom rate not set' }, 503);
    amount = Number(unitRow.flat_day_rate);
  } else {
    const nights = nightsBetween(checkIn, checkOut);
    if (nights < unitRow.min_nights) {
      return json({ error: `minimum ${unitRow.min_nights} nights` }, 400);
    }
    amount = nights * Number(unitRow.nightly_rate);
  }

  // Attempt the reservation; retry for 'any-room' if the chosen room is taken
  // by a concurrent booking between the pre-check and the insert.
